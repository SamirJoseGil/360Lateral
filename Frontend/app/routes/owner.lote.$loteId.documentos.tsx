...
import RequiredDocumentsNotice from "~/components/RequiredDocumentsNotice"; seState } from "react";
import { json, redirect } from "@remix-run/node";
import { useLoaderData, useParams, useActionData, Link, useNavigation } from "@remix-run/react";
import type { ActionFunctionArgs, LoaderFunctionArgs } from "@remix-run/node";
import { getUser } from "~/utils/auth.server";
...
export async function action({ request, params }: ActionFunctionArgs) {
    ...
    // Clonar el request para poder leer el formData múltiples veces
    ...
    try {
        // Obtener formData del request original
        const uploadFormData = await request.formData();
        const { document, headers } = await uploadDocument(request, uploadFormData);
