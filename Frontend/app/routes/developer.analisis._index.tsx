...
export async function loader({ request }: LoaderFunctionArgs) {
    ...
    try {
        ...
        if (analisisRes.ok) {
            ...
            console.log(`[developer.analisis] Loaded ${analisis.length} análisis`);
        } else {
            console.error(`[developer.analisis] Error loading: ${analisisRes.status}`);
        }
...
export default function DeveloperAnalisisIndex() {
                ...
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">
                            Solicita y consulta análisis de aprovechamiento urbanístico de tus lotes favoritos
                        </p>
                    </div>
                    <Link
                        to="/developer/analisis/solicitar"
                        className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-lg hover:from-indigo-700 hover:to-indigo-800 transition-all shadow-lg hover:shadow-xl flex items-center gap-2"
                    >
            ...
            {analisisList.length === 0 ? (
                <div className="bg-white rounded-lg shadow p-12 text-center">
                    ...
                    <p className="text-sm text-gray-500 mt-2">
                        Comienza solicitando un análisis urbanístico para uno de tus lotes favoritos
                    </p>
                    <div className="mt-6">
                        <Link
                            to="/developer/analisis/solicitar"
                            className="inline-flex items-center px-6 py-3 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {analisisList.map((item: any) => (
                        <Link
                            key={item.id}
                            to={`/developer/analisis/${item.id}`}
                            className="bg-white rounded-lg shadow hover:shadow-lg transition-shadow p-6 border-2 border-transparent hover:border-indigo-200"
                        >
                            {/* Header */}
